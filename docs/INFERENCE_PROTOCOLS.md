...
| Anthropic Claude/Opus/Fable (direct API key, or subscription OAuth via the local CLI proxy) | Anthropic Messages | replay signed `thinking`/`redacted_thinking`, `tool_use`, and `tool_result` blocks unchanged | Chat Completions adapter | Messages is Claude's native structured protocol; Chat translation can discard signed thinking blocks. The CLI-proxy route preserves block replay |
| OpenAI Codex OAuth / xAI Grok Build OAuth via the local CLI proxy | Responses (stateless) | replay prior output items (including reasoning items) in `input`; `previous_response_id` is rejected — the CLI proxy is stateless | Chat Completions | The CLI proxy exposes native Responses and preserves provider reasoning items on replay, but holds no server-side session state |
...
| Z.AI GLM thinking models | Chat Completions with `reasoning_content` replay | preserve the complete assistant message, including `reasoning_content` and `tool_calls` | ordinary Chat Completions | GLM returns `reasoning_content` and accepts full-message replay (verified live) — the same stateless contract as Kimi |
| MiniMax and other OpenAI-compatible models without advertised native state | Chat Completions | visible transcript only (MiniMax inlines `<think>` in `content`, so ordinary transcript replay already preserves it) | ordinary Chat Completions | Do not claim stateful reasoning without a provider contract and verified route |
| Other OAuth/CLI adapters | Chat Completions unless native support is explicitly advertised | adapter-defined | Chat Completions | Subscription OAuth credentials are not automatically valid native API credentials |
