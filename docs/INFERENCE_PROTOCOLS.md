...
| Anthropic Claude/Opus/Fable via the local CLI-proxy OAuth route | Anthropic Messages | replay signed `thinking`/`redacted_thinking`, `tool_use`, and `tool_result` blocks unchanged | Chat Completions adapter | The CLI-proxy route is Claude's native structured protocol; Chat translation can discard signed thinking blocks |
