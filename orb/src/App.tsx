          ...
          <Show
            ...
            fallback={
              <>
                ...
                <For each={projects}>
                  {(project) => {
                    const kids = () => project.agents.length + project.folders.length > 0;
                    const isOpen = () => kids() && !collapsed[project.id];
                    return (
                      <div class={`group ${isOpen() ? "has" : ""}`}>
                        <div class="row-wrap">
                        <button
                          class="row project"
                          onClick={() => setCollapsed(project.id, !collapsed[project.id])}
                          onContextMenu={(e) =>
                            showCtx(e, [
                              { kind: "item", label: "New agent", icon: Ic.NewAgentIcon, onClick: () => { setNewProject(project.id); open(null); } },
                              { kind: "item", label: "New folder", icon: Ic.FolderIcon, onClick: () => setNameDlg({ kind: "folder", pid: project.id, value: "" }) },
                              { kind: "sep" },
                              {
                                kind: "item",
                                label: "Archive All",
                                ...
                                onClick: () =>
                                  setProjects(
                                    produce((ps) => {
                                      const p = ps.find((x) => x.id === project.id);
                                      if (p) for (const a of p.agents) a.archived = true;
                                    }),
                                  ),
                              },
                              {
                                ...
                                onClick: () =>
                                  setProjects(
                                    produce((ps) => {
                                      const i = ps.findIndex((x) => x.id === project.id);
                        ...
                        >
                          ...
                          <span
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              setPlusFor(plusFor() === project.id ? null : project.id);
                            }}
                          >
                            <Ic.PlusIcon size={14} />
                          </span>
                        </button>
                          <Show when={plusFor() === project.id}>
                            <div class="menu plus-menu" onPointerDown={(e) => e.stopPropagation()}>
                              <MenuList
                                items={[
                                  { kind: "item", label: "New agent", icon: Ic.NewAgentIcon, onClick: () => { setNewProject(project.id); open(null); } },
                                  { kind: "item", label: "New folder", icon: Ic.FolderIcon, onClick: () => setNameDlg({ kind: "folder", pid: project.id, value: "" }) },
                                ]}
                                onPick={() => setPlusFor(null)}
                              />
                            </div>
                          </Show>
                        </div>
                        <Show when={isOpen()}>
                          <For each={project.agents.filter((a) => !a.archived)}>
                            {(a) => (
                              <button
                                class={`row agent ${selected() === a.id ? "active" : ""}`}
                                onClick={() => open(a.id)}
                                onContextMenu={(e) =>
                                  showCtx(e, [
                                    { kind: "item", label: "Rename", icon: Ic.PencilIcon, onClick: () => setNameDlg({ kind: "rename-agent", pid: project.id, agentId: a.id, value: a.title }) },
                                    { kind: "item", label: "Archive", icon: Ic.ArchiveIcon, onClick: () => mutate(a.id, (x) => (x.archived = true)) },
                                    { kind: "sep" },
                                    {
                                      kind: "item",
                                      label: "Remove from Sidebar",
                                      ...
                                      onClick: () =>
                                        setProjects(
                                          produce((ps) => {
                                            const p = ps.find((x) => x.id === project.id);
                                            if (!p) return;
                                            const i = p.agents.findIndex((x) => x.id === a.id);
                                            if (i >= 0) p.agents.splice(i, 1);
                                          }),
                                        ),
                                    },
                                  ])
                              ...
                              >
                                <span class="glyph">
                                  <StatusGlyph agent={a} busy={streamingId() === a.id} />
                                </span>
                          ...
                          <For each={project.folders}>
                            {(folder) => {
                              const openFolder = () => folder.files.length > 0 && !collapsed[`${project.id}/${folder.id}`];
                              return (
                                <div>
                                  <button
                                    class="row folder"
                                    onClick={() => setCollapsed(`${project.id}/${folder.id}`, openFolder())}
                                    onContextMenu={(e) =>
                                      showCtx(e, [
                                        { kind: "item", label: "New file", icon: Ic.FileIcon, onClick: () => setNameDlg({ kind: "file", pid: project.id, fid: folder.id, value: "" }) },
                                        { kind: "item", label: "Rename", icon: Ic.PencilIcon, onClick: () => setNameDlg({ kind: "rename-folder", pid: project.id, fid: folder.id, value: folder.name }) },
                                        { kind: "sep" },
                                        {
                                          kind: "item",
                                          label: "Remove from Sidebar",
                                          ...
                                          onClick: () =>
                                            setProjects(
                                              produce((ps) => {
                                                const p = ps.find((x) => x.id === project.id);
                                                if (!p) return;
                                                const i = p.folders.findIndex((x) => x.id === folder.id);
                                                if (i >= 0) p.folders.splice(i, 1);
                                              }),
                                            ),
                                        },
                                      ])
                                  ...
                                  >
                                    <Show when={openFolder()} fallback={<Ic.FolderIcon />}>
                                      <Ic.FolderOpenIcon />
                                    </Show>
                                    ...
                                    <span
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        setNameDlg({ kind: "file", pid: project.id, fid: folder.id, value: "" });
                                      }}
                                    >
                                      <Ic.PlusIcon size={14} />
                                    </span>
                                  ...
                                  <Show when={openFolder()}>
                                    <For each={folder.files}>
                                      {(file) => (
                                        <button
                                          class={`row file ${selected() === `f:${project.id}:${folder.id}:${file.id}` ? "active" : ""}`}
                                          onClick={() => {
                                            setMdSrc(false);
                                            open(`f:${project.id}:${folder.id}:${file.id}`);
                                          }}
                                          onContextMenu={(e) =>
                                            showCtx(e, [
                                              { kind: "item", label: "Rename", icon: Ic.PencilIcon, onClick: () => setNameDlg({ kind: "rename-file", pid: project.id, fid: folder.id, fileId: file.id, value: file.name }) },
                                              { kind: "sep" },
                                              {
                                                kind: "item",
                                                label: "Remove from Sidebar",
                                                ...
                                                onClick: () =>
                                                  setProjects(
                                                    produce((ps) => {
                                                      const f = ps.find((x) => x.id === project.id)?.folders.find((x) => x.id === folder.id);
                                                      if (!f) return;
                                                      const i = f.files.findIndex((x) => x.id === file.id);
                                                      if (i >= 0) f.files.splice(i, 1);
                                                    }),
                                                  ),
                                              },
                                            ])
