...
export function NativeInteraction(p:{mission:string;active:boolean;remote?:boolean;items?:StreamItem[]}) {
 ...
 return <Show when={request()}>{r=><section class="native-question" aria-label="Waiting for your response">
  <div class="native-question-heading">{r().method==='plan'?'Review the plan':r().method==='permission'?'Permission requested':'Waiting for your response'}</div>
  <Show when={r().method==='plan'||r().method==='permission'} fallback={<For each={r().params.questions}>{(q,i)=>{
   ...
   return <fieldset disabled={sending()}><legend>{q.question}</legend>
    <For each={q.options}>{option=><label><input type={q.multiSelect?'checkbox':'radio'} name={`question-${key()}`} checked={answers()[key()]?.includes(option.label)??false} onChange={e=>setAnswers(prev=>({...prev,[key()]:q.multiSelect?(e.currentTarget.checked?[...(prev[key()]??[]),option.label]:(prev[key()]??[]).filter(v=>v!==option.label)):[option.label]}))}/><span>{option.label}<small>{option.description}</small></span></label>}</For>
    <input class="s-input" aria-label={`Other answer: ${q.question}`} placeholder="Your answer…" onInput={e=>setAnswers(prev=>({...prev,[key()]:[e.currentTarget.value]}))}/>
   </fieldset>;
  }}</For>}>
   <Show when={r().params.plan}><div class="native-plan"><MdView compact text={r().params.plan!}/></div></Show>
   <Show when={r().method==='permission'}><p>{r().params.tool ?? 'Tool action'}</p><pre>{r().params.input?.description ?? r().params.input?.command ?? r().params.input?.file_path ?? 'The agent is requesting permission to continue.'}</pre></Show>
   <textarea aria-label="Requested changes" placeholder={r().method==='plan'?"Changes to the plan…":"Optional feedback…"} value={feedback()} onInput={e=>setFeedback(e.currentTarget.value)}/>
  </Show>
  <div class="native-question-actions"><Show when={r().method==='plan'||r().method==='permission'} fallback={<button class="pill" disabled={sending()||(r().params.questions??[]).some((q,i)=>!(answers()[q.id??String(i)]??[]).some(v=>v.trim()))} onClick={()=>void reply()}>Send answer</button>}>
   <button class="pill" disabled={sending()||(r().method==='plan'&&!feedback().trim())} onClick={()=>void reply('revise')}>{r().method==='plan'?'Request changes':'Decline'}</button>
   <button class="pill" disabled={sending()} onClick={()=>void reply('accept')}>{r().method==='plan'?'Implement plan':'Allow once'}</button>
