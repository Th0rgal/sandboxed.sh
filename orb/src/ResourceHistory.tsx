...
export type ResourceSample = { time: number; gpu?: number | null; cpu?: number | null; memory?: number | null };
...
export function ResourceHistory(p: { samples: ResourceSample[]; live: boolean }) {
  ...
  const path = (key: "cpu" | "memory" | "gpu") => {
    ...
    return p.samples.map(s => {
      ...
      previous = s.time;
      return `${command}${((s.time - (end() - 60000)) / 60000 * 600).toFixed(1)},${(72 - Math.min(100, Math.max(0, v)) / 100 * 68).toFixed(1)}`;
    }).join(" ");
  };
  ...
  return <div class="resource-history">
    ...
    <svg viewBox="0 0 600 80" preserveAspectRatio="none" role="img" aria-label="CPU and memory usage over the last minute">
      <path d="M0 4H600 M0 38H600 M0 72H600" class="history-grid" />
      <Show when={cpu()}><path d={path("cpu")} class="history-cpu" /></Show><path d={path("memory")} class="history-memory" /><Show when={p.samples.some(s => s.gpu != null)}><path d={path("gpu")} class="history-gpu" /></Show>
      <Show when={p.samples.length === 1}><circle cx="600" cy={72 - (p.samples[0].memory ?? 0) / 100 * 68} r="2" class="history-dot" /></Show>
    </svg>
    <div class="history-legend"><span>−1 min</span><span class="history-keys"><Show when={cpu()}><i class="cpu-key" />CPU</Show><i />RAM<Show when={p.samples.some(s => s.gpu != null)}><i class="gpu-key" />GPU</Show></span><span>Latest · 0–100%</span></div>
  </div>;
}
