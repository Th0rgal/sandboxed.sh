                ...
                result = rx.recv() => {
                    match result {
                        ...
                        Err(broadcast::error::RecvError::Lagged(dropped)) => {
                            ...
                            // `error` as fatal (toast + system row in the
                            // chat) and it caused a confusing user-facing
                            // alert every time a chatty mission (text_delta
                            // burst, big tool result) outpaced the
                            // browser tab's event handler.
                            //
                            ...
                            // so any events that got dropped on the floor
                            // are recovered from the database. No user
                            // toast, no chat-row pollution.
                            tracing::warn!(
                                stream_id = %stream_id,
                                dropped = dropped,
